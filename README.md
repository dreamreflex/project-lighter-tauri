...
- 一键启动 / 一键关闭全部项目；可配置启动前检查并释放的 TCP 端口列表。
- 命令、脚本两种项目类型；多行脚本每轮执行一次，支持单独执行或一键启动。
...
测试覆盖配置兼容和校验、备份、ANSI 分块解析、实际进程执行、失败中止、子进程停止、端口查询及原生界面基本交互。Linux 已在本地验证；Windows 构建、测试和打包也通过 GitHub Actions 验证；尚未进行 Windows 桌面实机验收。
