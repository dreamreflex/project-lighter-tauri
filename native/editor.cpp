...
ProjectEditor::ProjectEditor(const Project &project, QWidget *parent) : QDialog(parent), original_(project) {
    ...
    form->addRow(QStringLiteral("项目名称 *"), name_); form->addRow(QStringLiteral("工作目录"), directoryRow); layout->addLayout(form);
    ...
    actions->addWidget(add); actions->addWidget(remove); actions->addStretch(); actions->addWidget(up); actions->addWidget(down); layout->addLayout(actions);
...
Project ProjectEditor::project() const {
    ...
    for (int row = 0; row < commands_->rowCount(); ++row) result.commands.append({commands_->item(row, 0)->text().trimmed(), commands_->item(row, 1)->text().trimmed()});
    return result;
}
